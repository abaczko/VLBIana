...
files   = [eh.image.load_fits(m,aipscc=True) for m in vlba_maps] # maps loaded with ehtim 
...
for i in range(len(files)-1):
	eht_maps=[files[i],files[i+1]]
	maps=[vlba_maps[i],vlba_maps[i+1]]
	if i==0:
		rr=plot_aligned_maps(eht_maps,maps,cut_left=50)
	elif i==1:
		rr=plot_aligned_maps(eht_maps,maps,e_maj=100,e_min=60,e_xoffset=10)
