...
bbox_props = dict(boxstyle="square,pad=0.3",fc='white',lw=0.5)
...
#   
    ...
    if args['plot_map']:
        ...
        if np.logical_or(xs>1,ys>1):
            figsize=set_size(args['fig_size'],subplots=(ys,xs),ratio=0.54)
        ...
        for img in mapFile:
            ...
            if np.logical_and(xs>1,ys>1):
                ...
                ax[k,l].invert_xaxis()
                ax[k,l].annotate(label[i], xy=(0.65,0.9),xycoords='axes fraction',fontsize=args['asize'],bbox=bbox_props,zorder=20)

                plotBeam(maps_beam[i][1],maps_beam[i][0],maps_beam[i][2],ra_max[i],dec_min[i],ax[k,l])
                cntr=ax[k,l].contour(xx[i],yy[i],cleanmap,linewidths=0.5,levels=lev[i],colors='k',alpha=1,zorder=1) #,extent=extent[i],origin='lower',alpha=1)
                cntr.set_norm(norm)
                if args['overplot_rl']:
                    ax[k,l].scatter(RAm[i],Decm[i],c='red',s=1,marker='.',zorder=4)
                    ax[k,l].errorbar(RAm[i],Decm[i],yerr=Dec_error[i],xerr=RA_error[i],fmt='none',c='red',alpha=0.5,elinewidth=0.4,zorder=3)
                ...
                cntr=ax[k].contour(xx[i],yy[i],cleanmap,linewidths=0.5,levels=lev[i],colors='grey',alpha=1,zorder=1) #,extent=extent[i],origin='lower',alpha=1)
                cntr.set_norm(norm)
                if args['overplot_rl']:
                    ax[k].scatter(RAm[i],Decm[i],c='red',s=1,marker='.',zorder=4)
                    ax[k].errorbar(RAm[i],Decm[i],yerr=Dec_error[i],xerr=RA_error[i],fmt='none',c='red',alpha=0.5,elinewidth=0.4,zorder=3)

            #   if args['plot_stacked']:
            #       ax[k].annotate('VLBA {0:.1f} GHz stacked'.format(freqs[i]), xy=(0.5,0.8),xycoords='axes fraction',color='black',fontsize=args['asize'],bbox=bbox_props,zorder=20)
            #   else:
                ax[k].annotate(label[i], xy=(0.58,0.9),xycoords='axes fraction',color='black',fontsize=args['asize'],bbox=bbox_props,zorder=20)
            else:
                ax.axis(axe_ratio)
                ...
                ax.annotate(label[i], xy=(0.65,0.9),xycoords='axes fraction',color='black',fontsize=args['asize']+3,bbox=bbox_props)
